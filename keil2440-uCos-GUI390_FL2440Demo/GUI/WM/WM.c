	...
	if (hWin)
	{
		...
		if (Status & WM_SF_ISVIS)
		{
			...
			if (WM__ClipAtParentBorders(&r, hWin))
			{
				...
				if ((Status & (WM_SF_HASTRANS | WM_SF_CONST_OUTLINE)) ==
					WM_SF_HASTRANS)
				{
...
WM_HWIN WM_CreateWindow(int x0,
						...
						int NumExtraBytes)
{
	return WM_CreateWindowAsChild(x0,
								  y0,
								  width,
								  height,
								  0 /* No parent */,
...
static int _FindNext_IVR(void)
{
	...
	*/
	...
	{
		/* First IVR starts in upper left */
...
int WM__InitIVRSearch(const GUI_RECT *pMaxRect)
{
	...
	{
		/* Not using callback mechanism, therefor allow entire rectangle */
...
static int _Paint1Trans(WM_HWIN hWin, WM_Obj *pWin)
{
	...
	if (GUI_RectsIntersect(&pAWin->InvalidRect, &pWin->Rect))
	{
		...
		GUI__IntersectRects(&pWin->InvalidRect,
							&pWin->Rect,
							&pAWin->InvalidRect);
...
static void _PaintTransTopSiblings(WM_HWIN hWin, WM_Obj *pWin)
{
	...
	while (hParent)
	{
		...
		for (; hWin; hWin = pWin->hNext)
		{
			...
			if ((pWin->Status & (WM_SF_HASTRANS | WM_SF_ISVIS)) ==
				(WM_SF_HASTRANS | WM_SF_ISVIS))
			{
...
void WM__PaintWinAndOverlays(WM_PAINTINFO *pInfo)
{
	...
	if ((pWin->Status & (WM_SF_HASTRANS | WM_SF_CONST_OUTLINE)) !=
		WM_SF_HASTRANS)
	{
#endif
		...
		if (pWin->cb)
		{
			if (WM__ClipAtParentBorders(&pWin->InvalidRect, hWin))
			{
				...
				if (pWin->Status & WM_SF_MEMDEV)
				{
					...
					Flags = (pWin->Status & WM_SF_HASTRANS) ?
							GUI_MEMDEV_HASTRANS :
							GUI_MEMDEV_NOTRANS;
					/*
...
void WM_Init(void)
{
	if (!_IsInited)
	{
		...
		/* Make sure we have at least one window. This greatly simplifies the
			  ...
			  */
#if GUI_NUM_LAYERS == 1
		WM__ahDesktopWin[0] = WM_CreateWindow(0,
											  0,
											  GUI_XMAX,
											  GUI_YMAX,
											  WM_CF_SHOW,
...
#else
		{
			...
			for (i = 0; i < GUI_NUM_LAYERS; i++)
			{
				WM__ahDesktopWin[i] = WM_CreateWindowAsChild(0,
															 0,
															 GUI_XMAX,
															 GUI_YMAX,
															 WM_UNATTACHED,
