	...
	for (hChild = pObj->hFirstChild; hChild; hChild = pChild->hNext)
	{
		...
		switch (Status)
		{
			case WM_SF_ANCHOR_RIGHT:
				/* Right ANCHOR : Move window with right side */
				rNew.x0 += dx1;
				rNew.x1 += dx1;
				break;
			case WM_SF_ANCHOR_RIGHT | WM_SF_ANCHOR_LEFT:
				/* Left & Right ANCHOR: Resize window */
				rNew.x0 += dx0;
				rNew.x1 += dx1;
				break;
			default:
				/* Left ANCHOR: Move window with left side of parent */
				rNew.x0 += dx0;
				rNew.x1 += dx0;
		}
...
#else
void WM_UpdateChildPositions_C(void)
{
