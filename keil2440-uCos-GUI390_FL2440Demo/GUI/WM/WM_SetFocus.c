...
int WM_SetFocus(WM_HWIN hWin)
{
	...
	if ((hWin) && (hWin != WM__hWinFocus))
	{
		...
		if ((r = Msg.Data.v) == 0)
		{
			...
			if (WM_IsWindow(hWin))
			{
				/* Make sure window has not been deleted in the mean time. Can be optimized: _DeleteWindow could clear the handle to avoid this check (RS) */
				while ((hWin = WM_GetParent(hWin)) != 0)
				{
					Msg.MsgId = WM_NOTIFY_CHILD_HAS_FOCUS;
					Msg.Data.p = &Info;
