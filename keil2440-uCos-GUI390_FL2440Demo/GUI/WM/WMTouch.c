...
{
	if ((WM__CHWinModal.hWin == 0) ||
		WM__IsAncestor(hWin, WM__CHWinModal.hWin) ||
		(hWin == WM__CHWinModal.hWin))
	{
		return 1;
	}
	return 0;
		...
		(WM_PID__StateLast.Pressed != StateNew.Pressed))
	{
		...
		if (WM__IsInModalArea(CHWin.hWin))
		{
			...
			/*
					 ...
					 */
			if (WM_PID__StateLast.Pressed | StateNew.Pressed)
			{
				/* Only if pressed or just released */
				Msg.MsgId = WM_TOUCH;
				r = 1;
				/*
				...
				if (WM__CHWinLast.hWin != CHWin.hWin)
				{
					if (WM__CHWinLast.hWin != 0)
					{
						...
						GUI_DEBUG_LOG1("\nSending WM_Touch to LastWindow %d (out of area)",
									   WM__CHWinLast.hWin);
