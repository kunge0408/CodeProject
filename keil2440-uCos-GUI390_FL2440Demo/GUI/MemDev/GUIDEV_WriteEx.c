...
*
*/
static int _GetPixelIndex(const U8 *pData,
						  int x,
						  int y,
						  int bpp,
						  int BytesPerLine)
...
*   	_DrawBitmapLineEx
*/
static void _DrawBitmapLineEx(int x0,
							  int y0,
							  int xOff,
							  int yOff,
							  int xSize,
...
*   	_DrawBitmapEx
*/
static void _DrawBitmapEx(int x0,
						  int y0,
						  int xOff,
						  int yOff,
						  int xSize,
...
*   	_WriteExToActiveAt
*/
static void _WriteExToActiveAt(GUI_MEMDEV_Handle hMem,
							   int x,
							   int y,
							   int xMag,
							   int yMag,
		...
		for (yi = 0; yi < ySize; yi++)
		{
			...
			if (yAct != yPrev)
			{
				...
				if (xSize == pDev->XSize)
				{
					...
					y0 = yi;
					while (GUI_USAGE_GetNextDirty(pUsage, &xOff, yi + 1) ==
						   xSize)
					{
						yi++;
					}
