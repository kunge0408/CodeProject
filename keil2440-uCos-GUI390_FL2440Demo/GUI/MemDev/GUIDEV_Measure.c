...
{
	GUI_MEASDEV* pDev = (GUI_MEASDEV *)
						(GUI_MEMDEV_H2P(GUI_Context.hDevData));
	if (x < pDev->rUsed.x0)
	{
		pDev->rUsed.x0 = x;
	}
...
*   	_CalcPolyRect
*/
static void _CalcPolyRect(GUI_RECT *pr,
						  const GUI_POINT *paPoint,
						  int NumPoints)
{
	int i;
...
*   	_FillPolygon
*/
static void _FillPolygon(const GUI_POINT *paPoint,
						 int NumPoints,
						 int x0,
						 int y0)
{
...
*   	_FillPolygonAA
*/
static void _FillPolygonAA(const GUI_POINT *paPoint,
						   int NumPoints,
						   int x0,
						   int y0)
{
