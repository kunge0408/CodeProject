...
*/
static const LCD_PIXELINDEX aID[] = { 0,
									  1,
									  2,
									  3,
									  4,
...
static U8 * _XY2PTR_BITOFFSET(int x, int y, int *pBitOffset)
{
	...
	x -= pDev->x0;
	y -= pDev->y0;
	Offset = (GUI_ALLOC_DATATYPE_U)
			 (y) * (GUI_ALLOC_DATATYPE_U)
			 (pDev->BytesPerLine) +
			 (x >> 3);
	if (pBitOffset)
	{
...
*   	_DrawBitLine1BPP
*/
static void _DrawBitLine1BPP(GUI_USAGE *pUsage,
							 int x,
							 int y,
							 const U8 GUI_UNI_PTR *p,
							 int Diff,
							 unsigned int xsize,
							 ...
							 PIXELINDEX *pDest)
{
	...
	PixelCnt = 8 - (Diff & 7);
	pixels = (*p) << (Diff & 7);
	GUI_DEBUG_ERROROUT3_IF(x <
						   pDev->x0,
						   "GUIDEV.c: DrawBitLine1BPP, Act= %d, Border= %d, Clip= %d"
					,
						   x,
						   pDev->x0,
						   GUI_Context.ClipRect.x0);
	switch (GUI_Context.DrawMode & (LCD_DRAWMODE_TRANS | LCD_DRAWMODE_XOR))
	{
		case 0:
			...
			do
			{
				...
				do
				{
					(*pDev->pAPIList->pfSetPixelIndex)
					(x++, y, * (pTrans + ((U8) pixels >> 7)));
					pixels <<= 1;
				}
				while (--PixelCnt);
...
*   	_DrawBitLine2BPP
*/
static void _DrawBitLine2BPP(GUI_USAGE *pUsage,
							 int x,
							 int y,
							 const U8 GUI_UNI_PTR *p,
							 int Diff,
...
*   	_DrawBitLine4BPP
*/
static void _DrawBitLine4BPP(GUI_USAGE *pUsage,
							 int x,
							 int y,
							 const U8 GUI_UNI_PTR *p,
							 int Diff,
...
*   	_DrawBitLine8BPP
*/
static void _DrawBitLine8BPP(GUI_USAGE *pUsage,
							 int x,
							 int y,
...
*/
static void _DrawBitLine8BPP_DDB(GUI_USAGE *pUsage,
								 int x,
								 int y,
								 const U8 GUI_UNI_PTR *pSrc,
								 int xsize,
...
*   	_DrawBitmap
*/
static void _DrawBitmap(int x0,
						int y0,
						int xsize,
						int ysize,
						int BitsPerPixel,
...
static void _FillRect(int x0, int y0, int x1, int y1)
{
	...
	if (pDev->hUsage)
	{
		GUI_USAGE_AddRect(GUI_USAGE_H2P(pDev->hUsage),
						  x0,
						  y0,
						  Len,
						  y1 - y0 + 1);
