...
static int _GetNumCharAreas(const GUI_FONT GUI_UNI_PTR *pFont)
{
	U16 NumCharAreas;
	const U8 * pData;
	pData = (const U8 *) pFont->p.pFontData +
			14 /* Skip ID, YSize, YDist, Baseline, LHeight and CHeight */;
...
static const U8 * _GetpCharInfo(const GUI_FONT GUI_UNI_PTR *pFont, U16P c)
{
	...
	pCharArea = (const U8 *) pFont->p.pFontData + sizeof(GUI_SI_FONT);  					/* Set char area pointer to first char area */
	pCharInfo = pCharArea + sizeof(GUI_SIF_CHAR_AREA) * NumCharAreas; /* Set char info pointer to first character info */
	do
	{
		...
		pCharInfo += sizeof(GUI_SIF_CHARINFO) * (CharArea.Last -
												 CharArea.First +
												 1);
	}
	while (--NumCharAreas);
...
static void _GUI_SIF_DispChar(U16P c)
{
	...
	if (pCharInfo)
	{
		...
		pData = (const U8 *) GUI_Context.pAFont->p.pFontData +
				CharInfo.OffData;
...
*   	GUI_SIF_GetFontInfo
*/
static void _GUI_SIF_GetFontInfo(const GUI_FONT GUI_UNI_PTR *pFont,
								 GUI_FONTINFO *pfi)
{
	const U8 * pData;
