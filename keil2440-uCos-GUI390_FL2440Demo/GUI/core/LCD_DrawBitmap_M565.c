...
*   	_DrawBitmap_M565
*/
static void _DrawBitmap_M565(int x0,
							 int y0,
							 int xsize,
							 int ysize,
							 ...
							 int yMag)
{
	...
	if (GUI_Context.pDeviceAPI->pMemDevAPI)
	{
		if (((GUI_Context.hDevData &&
			  (GUI_Context.pDeviceAPI->pMemDevAPI->pfIndex2Color ==
			   LCD_Index2Color_565)) ||
			 ((GUI_Context.hDevData == 0) && (Mode == 565) && (SwapRB == 1))) &&
			(Mag == 1))
		{
			UseDrawBitmap = 1;
		}
