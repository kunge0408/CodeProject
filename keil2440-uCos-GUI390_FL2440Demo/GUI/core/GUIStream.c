...
*/
void GUI_DrawStreamedBitmap(const GUI_BITMAP_STREAM *pBitmapHeader,
							int x,
							int y)
{
	GUI_BITMAP Bitmap = {0};
	...
	Bitmap.pData = ((const U8 *) pBitmapHeader) +
				   16 +
				   4 * pBitmapHeader->NumColors;
	Bitmap.pPal = &Palette;
	...
	Palette.pPalEntries = (const LCD_COLOR *)
						  ((const U8 *) pBitmapHeader + 16);
