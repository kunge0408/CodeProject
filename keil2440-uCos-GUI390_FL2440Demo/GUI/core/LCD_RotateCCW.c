...
static void _TransformPointCCW(int *pXPos, int *pYPos)
{
	...
	*pYPos = *pYPos +
			 ...
			 (ClientRect.y1 - ClientRect.y0) +
			 GUI_RectDispString.x0;
}
...
*   	_DrawBitmapCCW
*/
static void _DrawBitmapCCW(int x0,
						   int y0,
						   int xsize,
						   int ysize,
						   int xMul,
						   ...
						   const LCD_PIXELINDEX *pTrans)
{
	...
	else
	{
		...
		for (x = x0, xi = 0;
			 xi < ysize;
			 xi++, x += yMul, pPixel += BytesPerLine)
		{
			int xMax = x + yMul - 1;
			if ((xMax >= GUI_Context.ClipRect.x0) &&
				(x <= GUI_Context.ClipRect.x1))
			{
				int BitsLeft = 0;
...
(tLCD_DrawBitmap *) &_DrawBitmapCCW, &_Rect2TextRect
};
