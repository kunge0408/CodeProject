...
{
	return (size + ((1 << GUI_BLOCK_ALIGN) - 1)) & ~((1 << GUI_BLOCK_ALIGN) -
													 1);
}
...
static GUI_HMEM _FindFreeHandle(void)
{
	...
	GUI_DEBUG_ERROROUT1("Insufficient memory handles configured (GUI_MAXBLOCKS == %d (See GUIConf.h))",
						GUI_MAXBLOCKS);
...
static GUI_HMEM _FindHole(GUI_ALLOC_DATATYPE Size)
{
	...
	{
		int NumFreeBytes = aBlock[iNext].Off -
						   (aBlock[i].Off + aBlock[i].Size);
...
static GUI_HMEM _CreateHole(GUI_ALLOC_DATATYPE Size)
{
	...
	for (i = 0; (iNext = aBlock[i].Next) != 0; i = iNext)
	{
		GUI_ALLOC_DATATYPE NumFreeBytes = aBlock[iNext].Off -
										  (aBlock[i].Off + aBlock[i].Size);
		if (NumFreeBytes < Size)
		{
			GUI_ALLOC_DATATYPE NumBytesBeforeBlock = aBlock[iNext].Off -
													 (aBlock[i].Off +
													  aBlock[i].Size);
...
static GUI_HMEM _Alloc(GUI_ALLOC_DATATYPE size)
{
	...
	{
		GUI_DEBUG_WARN1("GUI_ALLOC_Alloc: Insufficient memory configured (Trying to alloc % bytes)",
						size);
		return 0;
	}
	/* Locate free handle */
	...
	if (hMemIns == -1)
	{
		GUI_DEBUG_ERROROUT1("GUI_ALLOC_Alloc: Could not allocate %d bytes",
							size);
		return 0;
	}
	{
