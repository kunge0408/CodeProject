...
void GUI_Unlock(void)
{
	...
	GUI_DEBUG_ERROROUT_IF((_EntranceCnt < 0),
						  "GUITASK.c: GUI_Unlock() _EntranceCnt underflow ");
}
...
void GUI_Lock(void)
{
	...
	/* Test if _EntranceCnt is in reasonable range ... Not required in release builds */
	GUI_DEBUG_ERROROUT_IF((_EntranceCnt > GUI_MAX_LOCK_NESTING),
						  "GUITASK.c: GUI_Lock() _EntranceCnt overflow ");
}
