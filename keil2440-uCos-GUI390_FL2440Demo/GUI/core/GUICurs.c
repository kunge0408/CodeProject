		...
		for (y = 0; y < ySize; y++)
		{
			for (x = 0; x < xSize; x++)
			{
				...
				*(pData + x) = _GetPixelIndex(_Rect.x0 + x, _Rect.y0 + y);
				...
				if (BitmapPixel)
				{
					_SetPixelIndex(_Rect.x0 + x,
								   _Rect.y0 + y,
								   _Log2Phys(BitmapPixel));
				}
			}
			pData += pBM->XSize;
		}
			...
			if (_CursorOn)
			{
				...
				for (y = yStart; y != yEnd; y += yStep)
				{
					...
					for (x = xStart; x != xEnd; x += xStep)
					{
						...
						int xNew = x + xOff;
						BitmapPixel = GUI_GetBitmapPixelIndex(pBM, x, y);
						xyOverlaps = (x >= xOverlapMin) &&
									 (x <= xOverlapMax) &&
									 yOverlaps;
						xyNewOverlaps = (xNew >= xOverlapMin) &&
										(xNew <= xOverlapMax) &&
										yNewOverlaps;
						...
						{
							if (!xyOverlaps ||
								(GUI_GetBitmapPixelIndex(pBM,
														 x - xOff,
														 y - yOff) == 0))
							{
								_SetPixelIndex(x + _Rect.x0,
											   y + _Rect.y0,
											   *(pSave));
							}
						}
						...
						{
							Pixel = _GetPixelIndex(_Rect.x0 + xNew,
												   _Rect.y0 + yNew);
						}
						*pSave = Pixel;
