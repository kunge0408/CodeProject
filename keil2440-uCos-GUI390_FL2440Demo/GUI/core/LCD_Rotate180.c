...
static void _TransformPoint180(int *pXPos, int *pYPos)
{
	...
	*pXPos = ClientRect.x0 +
			 GUI_RectDispString.x1 -
			 (xPos - GUI_RectDispString.x0);
	*pYPos = ClientRect.y0 +
			 GUI_RectDispString.y1 -
			 (yPos - GUI_RectDispString.y0);
}
...
*   	_DrawBitmap180
*/
static void _DrawBitmap180(int x0,
						   int y0,
						   int xsize,
						   int ysize,
						   int xMul,
						   ...
						   const LCD_PIXELINDEX *pTrans)
{
	...
	else
	{
		...
		for (y = y0, yi = 0;
			 yi < ysize;
			 yi++, y -= yMul, pPixel += BytesPerLine)
		{
			int yMax = y + yMul - 1;
			/* Draw if within clip area (Optimization ... "if" is not required !) */
			if ((yMax >= GUI_Context.ClipRect.y0) &&
				(y <= GUI_Context.ClipRect.y1))
			{
				int BitsLeft = 0;
				int xi;
				...
				for (x = x0, xi = 0; xi < xsize; xi++, x -= xMul)
				{
					...
					if (Index ||
						((GUI_Context.DrawMode & LCD_DRAWMODE_TRANS) == 0))
					{
...
(tLCD_DrawBitmap *) &_DrawBitmap180, &_Rect2TextRect
};
