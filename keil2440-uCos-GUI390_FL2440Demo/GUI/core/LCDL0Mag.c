...
*   	_DrawBitline
*/
static void _DrawBitline(int x0,
						 int y0,
						 int xsize,
						 int BPP,
						 const U8 *pData,
	...
	while (xsize > 0)
	{
		...
		switch (BPP)
		{
			...
			case 8:
				...
				{
#if LCD_XMAG > 2
					GUI_MEMSET(&Buffer.ab[i * LCD_XMAG],
							   *(pData + i),
							   LCD_XMAG);
#else
				...
				for (i = 0; i < NumPixels; i++)
				{
					...
					GUI_MEMSET(&Buffer.ab[i * LCD_XMAG],
							   *(pData + i),
							   LCD_XMAG);
#else
					Buffer.au16[i * LCD_XMAG] = Buffer.au16[i * LCD_XMAG + 1] = *(U16 *)
												pData;
		...
		LCD_L0_DrawBitmap(x0 + Diff * LCD_XMAG,
						  y0,
						  NumPixels * LCD_XMAG,
						  LCD_YMAG,
						  BPP,
...
*/
void LCD_L0_MAG_DrawBitmap(int x0,
						   int y0,
						   int xsize,
						   int ysize,
						   int BPP,
