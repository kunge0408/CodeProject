...
*   	GL_DrawBitmapEx
*/
static void GL_DrawBitmapEx(const GUI_BITMAP GUI_UNI_PTR *pBitmap,
							int x0,
							int y0,
							int xCenter,
							int yCenter,
							...
							int yMag)
{
	...
	x0 -= (I32)
		  ((xMag < 0) ? xSize - xCenter - 1 : xCenter) * (I32)
		  (xMagAbs) /
		  (I32)
		  (1000);
	...
	for (yi = 0; yi < ySize; yi++)
	{
		y = yEnd;
		yEnd = y0 +
			   GUI__DivideRound32(((I32) (yi + 1 - yCenter) * (I32) (yMag)),
									   1000);
		if (y != yPrev)
		{
			yPrev = y;
...
*   	GUI_DrawBitmapEx
*/
void GUI_DrawBitmapEx(const GUI_BITMAP GUI_UNI_PTR *pBitmap,
					  int x0,
					  int y0,
					  int xCenter,
					  int yCenter,
					  ...
					  int yMag)
{
	...
	{
		r.x0 = x0 +
			   GUI__DivideRound32(((I32) (-xCenter) * (I32) (xMag)), 1000);
		r.x1 = x0 +
			   GUI__DivideRound32(((I32)
								   (pBitmap->XSize - xCenter - 1) * (I32)
								   (xMag)),
								  1000);
	}
	else
	...
	{
		r.y0 = y0 +
			   GUI__DivideRound32(((I32) (-yCenter) * (I32) (yMag)), 1000);
		r.y1 = y0 +
			   GUI__DivideRound32(((I32)
								   (pBitmap->YSize - yCenter - 1) * (I32)
								   (yMag)),
								  1000);
	}
	else
