...
*   	_DrawBitmap_Pal
*/
static int _DrawBitmap_Pal(const U8 *pData,
						   int x0,
						   int y0,
						   int XSize,
						   int YSize,
...
*   	_DrawBitmap_24bpp
*/
static int _DrawBitmap_24bpp(const U8 *pData,
							 int x0,
							 int y0,
							 int XSize,
							 int YSize)
{
	...
	for (; (y < YSize) && (y >= 0); y += Step)
	{
		for (x = 0; x < XSize; x++)
		{
			...
			g = *(pColor + 1);
			r = *(pColor + 2);
			LCD_SetPixelIndex(x0 + x,
							  y0 + y,
							  LCD_Color2Index(((U32) b << 16) | (g << 8) | r));
		}
		pData += BytesPerLine;
	}
	return 0;
...
int GUI_BMP_Draw(const void *pBMP, int x0, int y0)
{
	...
	if ((NumColors > LCD_MAX_LOG_COLORS) || (Type != 0x4d42) ||  /* 'BM' */
