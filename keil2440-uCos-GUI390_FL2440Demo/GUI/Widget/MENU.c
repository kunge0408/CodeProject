...
static int _GetItemWidth(MENU_Handle hObj, MENU_Obj *pObj, unsigned Index)
{
	...
	{
		MENU_ITEM* pItem = (MENU_ITEM *) GUI_ARRAY_GetpItem(&pObj->ItemArray,
															Index);
		if ((pObj->Flags & MENU_SF_VERTICAL) ||
			!(pItem->Flags & MENU_IF_SEPARATOR))
		{
		...
		ItemWidth += pObj->Props.aBorder[MENU_BI_LEFT] +
					 pObj->Props.aBorder[MENU_BI_RIGHT];
	}
	return ItemWidth;
}
...
static int _GetItemHeight(MENU_Handle hObj, MENU_Obj *pObj, unsigned Index)
{
	...
	else
	{
		...
		if (pObj->Flags & MENU_SF_VERTICAL)
		{
			MENU_ITEM* pItem = (MENU_ITEM *)
							   GUI_ARRAY_GetpItem(&pObj->ItemArray,
																Index);
		...
		ItemHeight += pObj->Props.aBorder[MENU_BI_TOP] +
					  pObj->Props.aBorder[MENU_BI_BOTTOM];
	}
	return ItemHeight;
}
...
*   	_GetItemPos
*/
static void _GetItemPos(MENU_Handle hObj,
						MENU_Obj *pObj,
						unsigned Index,
						int *px,
						int *py)
	...
	if (pObj->Flags & MENU_SF_ACTIVE)
	{
		...
		if (pItem->hSubmenu)
		{
			if ((pItem->Flags & MENU_IF_DISABLED) == 0)
			{
				...
				if (pObj->Flags & MENU_SF_VERTICAL)
				{
					x += _CalcMenuSizeX(hObj, pObj) -
						 (_GetEffectSize(hObj, pObj) << 1);
					y -= EffectSize;
				}
				else
				{
					y += _CalcMenuSizeY(hObj, pObj) -
						 (_GetEffectSize(hObj, pObj) << 1);
					x -= EffectSize;
				}
...
*/
static char _HandlePID(MENU_Handle hObj,
					   MENU_Obj *pObj,
					   int x,
					   int y,
					   int Pressed)
...
*/
static void _ForwardPIDMsgToOwner(MENU_Handle hObj,
								  MENU_Obj *pObj,
								  WM_MESSAGE *pMsg)
{
	if (_IsTopLevelMenu(hObj, pObj) == 0)
	{
		...
		if (hOwner)
		{
			if (pMsg->Data.p)
			{
				...
				pState->x += WM_GetWindowOrgX(hObj) -
							 WM_GetWindowOrgX(hOwner);
				pState->y += WM_GetWindowOrgY(hObj) -
							 WM_GetWindowOrgY(hOwner);
			}
...
*   	_SetPaintColors
*/
static void _SetPaintColors(const MENU_Obj *pObj,
	...
	if (pObj->Flags & MENU_SF_VERTICAL)
	{
		...
		for (i = 0; i < NumItems; i++)
		{
			...
			if (pItem->Flags & MENU_IF_SEPARATOR)
			{
				...
				GUI_DrawHLine(FillRect.y0 + BorderTop + 1,
							  FillRect.x0 + 2,
							  FillRect.x1 - 2);
			}
			else
			{
				...
				WIDGET__FillStringInRect(pItem->acText,
										 &FillRect,
										 &TextRect,
										 &TextRect);
			}
	...
	else
	{
		...
		for (i = 0; i < NumItems; i++)
		{
			...
			if (pItem->Flags & MENU_IF_SEPARATOR)
			{
				GUI_ClearRectEx(&FillRect);
				GUI_SetColor(0x7C7C7C);
				GUI_DrawVLine(FillRect.x0 + BorderLeft + 1,
							  FillRect.y0 + 2,
							  FillRect.y1 - 2);
			}
			else
...
*/
void MENU__InvalidateItem(MENU_Handle hObj,
						  const MENU_Obj *pObj,
						  unsigned Index)
{
	GUI_USE_PARA(pObj);
...
*   	MENU__SetItem
*/
char MENU__SetItem(MENU_Handle hObj,
				   MENU_Obj *pObj,
				   unsigned Index,
				   const MENU_ITEM_DATA *pItemData)
{
	...
	if (GUI_ARRAY_SetItem(&pObj->ItemArray,
						  Index,
						  &Item,
						  sizeof(MENU_ITEM) + strlen(pText)) != 0)
	{
		MENU_ITEM* pItem = (MENU_ITEM *) GUI_ARRAY_GetpItem(&pObj->ItemArray,
															Index);
...
*/
int MENU__SendMenuMessage(MENU_Handle hObj,
						  WM_HWIN hDestWin,
						  U16 MsgType,
						  U16 ItemId)
{
