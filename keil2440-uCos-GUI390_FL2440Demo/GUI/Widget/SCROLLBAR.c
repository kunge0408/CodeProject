...
GUI_COLOR SCROLLBAR__aDefaultBkColor[2] = {SCROLLBAR_BKCOLOR0_DEFAULT,
										   SCROLLBAR_BKCOLOR1_DEFAULT};
GUI_COLOR SCROLLBAR__aDefaultColor[2] = {SCROLLBAR_COLOR0_DEFAULT,
										 SCROLLBAR_COLOR1_DEFAULT};
...
static void _CalcPositions(SCROLLBAR_Obj *pObj, SCROLLBAR_POSITIONS *pPos)
{
	...
	pPos->x0_Thumb = pPos->x1_LeftArrow +
					 1 +
					 GUI__DivideRound(xSizeMoveable * pObj->v,
															   NumItems -
															   pObj->PageSize);
...
*   	_OnTouch
*/
static void _OnTouch(SCROLLBAR_Handle hObj,
					 SCROLLBAR_Obj *pObj,
					 WM_MESSAGE *pMsg)
{
	SCROLLBAR_POSITIONS Pos;
...
*   	_OnSetScrollState 
*/
static void _OnSetScrollState(SCROLLBAR_Handle hObj,
							  SCROLLBAR_Obj *pObj,
...
{
	/* Invalidate the partner, since it is also affected */
...
static void _SCROLLBAR_Callback(WM_MESSAGE *pMsg)
{
	...
	switch (pMsg->MsgId)
	{
		...
		case WM_SET_SCROLL_STATE:
			_OnSetScrollState(hObj,
							  pObj,
							  (const WM_SCROLL_STATE *) pMsg->Data.p);
			break;
