...
static void _Paint(/*HEADER_Handle hObj, */HEADER_Obj *pObj)
{
	...
	{
		HEADER_COLUMN * pColumn = (HEADER_COLUMN *)
								  GUI_ARRAY_GetpItem(&pObj->Columns,
																	   i);
		...
		if (pColumn->hDrawObj)
		{
			...
			switch (pColumn->Align & GUI_TA_HORIZONTAL)
			{
				case GUI_TA_RIGHT:
					xOff = (pColumn->Width -
							GUI_DRAW__GetXSize(pColumn->hDrawObj));
					break;
				case GUI_TA_HCENTER:
					xOff = (pColumn->Width -
							GUI_DRAW__GetXSize(pColumn->hDrawObj)) /
			...
			{
				case GUI_TA_BOTTOM:
					yOff = ((Rect.y1 - Rect.y0 + 1) -
							GUI_DRAW__GetYSize(pColumn->hDrawObj));
					break;
				case GUI_TA_VCENTER:
					yOff = ((Rect.y1 - Rect.y0 + 1) -
							GUI_DRAW__GetYSize(pColumn->hDrawObj)) /
	...
	if ((y >= 0) && (y < WM_GetWindowSizeY(hObj)))
	{
		if (hObj)
		{
			...
			for (Index = 0; Index < NumColumns; Index++)
			{
				...
				pColumn = (HEADER_COLUMN *) GUI_ARRAY_GetpItem(&pObj->Columns,
															   Index);
				xPos += pColumn->Width;
...
*/
#if (HEADER_SUPPORT_DRAG)
static void _HandlePID(HEADER_Handle hObj,
					   HEADER_Obj *pObj,
					   int x,
					   int y,
					   int Pressed)
{
	...
	{
		int NewSize = HEADER_GetItemWidth(hObj, pObj->CaptureItem) +
					  x -
					  pObj->CapturePosX;
		if (NewSize >= 0)
		{
...
static void _OnMouseOver(HEADER_Handle hObj,
						 HEADER_Obj *pObj,
						 WM_MESSAGE *pMsg)
{
...
void HEADER_AddItem(HEADER_Handle hObj, int Width, const char *s, int Align)
{
	if (hObj)
	{
		...
		if (!Width)
		{
			...
			Width = GUI_GetStringDistX(s) +
					2 * (pObj->Widget.pEffect->EffectSize + _DefaultBorderH);
			GUI_SetFont(pFont);
		}
		Column.Width = Width;
		Column.Align = Align;
		...
		if (GUI_ARRAY_AddItem(&pObj->Columns,
							  &Column,
							  sizeof(HEADER_COLUMN) + strlen(s) + 1) == 0)
		{
			HEADER_COLUMN * pColumn;
			pObj = HEADER_H2P(hObj);
			pColumn = (HEADER_COLUMN *) GUI_ARRAY_GetpItem(&pObj->Columns,
...
void HEADER_SetItemText(HEADER_Handle hObj, unsigned int Index, const char *s)
{
	if (hObj)
	{
		...
		if (Index < GUI_ARRAY_GetNumItems(&pObj->Columns))
		{
			HEADER_COLUMN* pColumn;
			pColumn = (HEADER_COLUMN *)
					  GUI_ARRAY_ResizeItem(&pObj->Columns,
										   Index,
										   sizeof(HEADER_COLUMN) +
															 strlen(s));
			if (pColumn)
			{
				strcpy(pColumn->acText, s);
