...
3,	/* number of entries */
1, 	/* Has transparency */ & _ColorsCursor[0]
};
...
static void _SetResizeCursor(int Mode)
{
	...
	if (Mode)
	{
		...
		else
		{
			...
			if ((Direction == (FRAMEWIN_REPOS_X | FRAMEWIN_REPOS_Y)) ||
				!Direction)
			{
...
static void _ChangeWindowPosSize(FRAMEWIN_Handle hWin, int *px, int *py)
{
	...
	{
		dx = (_CaptureFlags & FRAMEWIN_REPOS_X) ?
			 (_CaptureX - *px) :
			 (*px - _CaptureX);
	}
	if (_CaptureFlags & FRAMEWIN_RESIZE_Y)
	{
		dy = (_CaptureFlags & FRAMEWIN_REPOS_Y) ?
			 (_CaptureY - *py) :
			 (*py - _CaptureY);
	}
...
#else
void FRAMEWIN_SetResizeable_c(void)
{
