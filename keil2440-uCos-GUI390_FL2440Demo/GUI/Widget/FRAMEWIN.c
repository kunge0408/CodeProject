...
*   	_OnTouch
*/
static void _OnTouch(FRAMEWIN_Handle hWin,
					 FRAMEWIN_Obj *pObj,
					 WM_MESSAGE *pMsg)
{
...
*   
*/
static void _OnChildHasFocus(FRAMEWIN_Handle hWin,
							 FRAMEWIN_Obj *pObj,
							 WM_MESSAGE *pMsg)
{
	if (pMsg->Data.p)
	{
		const WM_NOTIFY_CHILD_HAS_FOCUS_INFO * pInfo = (const WM_NOTIFY_CHILD_HAS_FOCUS_INFO *)
													   pMsg->Data.p;
		...
		{
			/* A child has lost the focus, we need to deactivate */
			_SetActive(hWin, 0);
...
void FRAMEWIN__CalcPositions(FRAMEWIN_Obj *pObj, POSITIONS *pPos)
{
	...
	for (hChild = pObj->Widget.Win.hFirstChild;
		 hChild;
		 hChild = pChild->hNext)
	{
								  ...
								  WM_CALLBACK *cb)
{
	...
	if (hObj)
	{
		...
		WIDGET__Init(&pObj->Widget,
					 Id,
					 WIDGET_STATE_FOCUSSABLE | FRAMEWIN_SF_TITLEVIS);
		...
		FRAMEWIN__CalcPositions(pObj, &Pos);
		pObj->hClient = WM_CreateWindowAsChild(Pos.rClient.x0,
											   Pos.rClient.y0,
											   Pos.rClient.x1 -
											   Pos.rClient.x0 +
											   1,
