...
void DROPDOWN__AdjustHeight(DROPDOWN_Handle hObj, DROPDOWN_Obj *pObj)
{
	...
	Height += pObj->Widget.pEffect->EffectSize +
			  2 * pObj->Props.TextBorderSize;
...
static void _DROPDOWN_Callback(WM_MESSAGE *pMsg)
{
	...
	switch (pMsg->MsgId)
	{
		...
		case WM_PID_STATE_CHANGED:
			if (IsExpandedBeforeMsg == 0)
			{
				/* Make sure we do not react a second time */
...
void DROPDOWN_Expand(DROPDOWN_Handle hObj)
{
	...
	if (hObj)
	{
		...
		if (pObj->hListWin == 0)
		{
			...
			hLst = LISTBOX_CreateAsChild(NULL,
										 WM_GetParent(hObj),
										 r.x0,
										 r.y0,
										 xSize,
