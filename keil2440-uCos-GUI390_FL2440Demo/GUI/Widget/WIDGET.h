...
//To make sure the types of your pointer and constant agree, 
//define a new type, and then use that to declare your pointer 
//and constant.  For example:	
...
typedef void (*t_funcPtr)(void);
typedef void (*t2_funcPtr)(const GUI_RECT *pRec);
typedef void (*t3_funcPtr)(GUI_RECT *pRect);

typedef  struct {
  t_funcPtr	 pfDrawUp ;
	t_funcPtr	 pfDrawDown ;
	t2_funcPtr pfDrawUpRect;
	t2_funcPtr pfDrawDownRect;
	t3_funcPtr pfGetRect;
	int EffectSize;
