...
GUI_COLOR   				 MULTIPAGE__DefaultBkColor[2] = { MULTIPAGE_BKCOLOR0_DEFAULT,
															  MULTIPAGE_BKCOLOR1_DEFAULT };
GUI_COLOR   				 MULTIPAGE__DefaultTextColor[2] = { MULTIPAGE_TEXTCOLOR0_DEFAULT,
...
*   	_AddScrollbar
*/
static void _AddScrollbar(MULTIPAGE_Handle hObj,
						  MULTIPAGE_Obj *pObj,
						  int x,
						  int y,
						  int w,
...
*   	_SetScrollbar
*/
static void _SetScrollbar(MULTIPAGE_Handle hObj,
						  MULTIPAGE_Obj *pObj,
						  int NumItems)
{
	SCROLLBAR_Handle hScroll;
...
static void _UpdatePositions(MULTIPAGE_Handle hObj, MULTIPAGE_Obj *pObj)
{
	...
	if (Width > rBorder.x1)
	{
		...
		x0 = (pObj->Align & MULTIPAGE_ALIGN_RIGHT) ?
			 (rBorder.x0) :
			 (rBorder.x1 - 2 * Size + 1);
		y0 = (pObj->Align & MULTIPAGE_ALIGN_BOTTOM) ?
			 (rBorder.y1) :
			 (rBorder.y0 - Size + 1);
	...
	WM_SetSize(pObj->hClient,
			   rBorder.x1 - rBorder.x0 + 1,
			   rBorder.y1 - rBorder.y0 + 1);
...
*   	_DrawTextItem
*/
static void _DrawTextItem(MULTIPAGE_Obj *pObj,
						  const char *pText,
						  unsigned Index,
	...
	if (pObj->Handles.NumItems > 0)
	{
		...
		for (i = 0; i < pObj->Handles.NumItems; i++)
		{
			...
			_DrawTextItem(pObj,
						  ...
						  x0,
						  w,
						  (pPage->Status & MULTIPAGE_STATE_ENABLED) ? 1 : 0);
		}    
...
*   	_ClickedOnMultipage
*/
static int _ClickedOnMultipage(MULTIPAGE_Handle hObj,
							   MULTIPAGE_Obj *pObj,
							   int x,
							   int y)
{
...
*   	_OnTouch
*/
static void _OnTouch(MULTIPAGE_Handle hObj,
					 MULTIPAGE_Obj *pObj,
					 WM_MESSAGE *pMsg)
{
...
static void _Callback(WM_MESSAGE *pMsg)
{
	...
	switch (pMsg->MsgId)
	{
		...
		case WM_WIDGET_SET_EFFECT:
			WIDGET_SetEffect(WM_GetScrollbarH(hObj),
							 (WIDGET_EFFECT const *) pMsg->Data.p);
		case WM_SIZE:
									...
									int Id)
{
	...
	if (hObj)
	{
		...
		Flags = WM_CF_SHOW |
				...
				WM_CF_ANCHOR_TOP |
				WM_CF_ANCHOR_BOTTOM;
		pObj->hClient = WM_CreateWindowAsChild(rClient.x0,
											   rClient.y0,
...
void MULTIPAGE_AddPage(MULTIPAGE_Handle hObj, WM_HWIN hWin, const char *pText)
{
	...
	if (hObj)
	{
		...
		if (!hWin)
		{
			...
			WM_Obj* pClient = WM_H2P(pObj->hClient);
			int i;
			for (hChild = pClient->hFirstChild;
				 hChild && !hWin;
				 hChild = pChild->hNext)
			{
				pChild = WM_H2P(hChild);
				...
				for (i = 0; i < pObj->Handles.NumItems; i++)
				{
					pPage = (MULTIPAGE_PAGE *)
							GUI_ARRAY_GetpItem(&pObj->Handles,
																  i);
		...
		if (hWin)
		{
			...
			if (GUI_ARRAY_AddItem(&pObj->Handles,
								  &Page,
								  sizeof(MULTIPAGE_PAGE) + strlen(pText)) == 0)
			{
				MULTIPAGE_PAGE* pPage;
				pPage = (MULTIPAGE_PAGE *)
						GUI_ARRAY_GetpItem(&pObj->Handles,
										   pObj->Handles.NumItems -
															  1);
	...
	if (hObj)
	{
		...
		if (pObj)
		{
			if ((int) Index < pObj->Handles.NumItems)
			{
				WM_HWIN hWin;
				MULTIPAGE_PAGE* pPage;
				pPage = (MULTIPAGE_PAGE *) GUI_ARRAY_GetpItem(&pObj->Handles,
															  Index);
				hWin = pPage->hWin;
	...
	if (hObj && pText)
	{
		...
		if (pObj)
		{
			if ((int) Index < pObj->Handles.NumItems)
			{
				...
				if (GUI_ARRAY_SetItem(&pObj->Handles,
									  Index,
									  &Page,
									  sizeof(MULTIPAGE_PAGE) + strlen(pText)))
				{
					pPage = (MULTIPAGE_PAGE *)
							GUI_ARRAY_GetpItem(&pObj->Handles,
																  Index);
	...
	if (hObj)
	{
		...
		if (pObj)
		{
			...
			WM_MoveTo(pObj->hClient,
					  rClient.x0 + pObj->Widget.Win.Rect.x0,
