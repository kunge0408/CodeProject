...
static void _Paint(EDIT_Obj *pObj, EDIT_Handle hObj)
{
		...
		if (pText)
		{
			...
			if ((pObj->EditMode != GUI_EDIT_MODE_INSERT) || (pObj->SelSize))
			{
				...
				if (pObj->CursorPos < NumChars)
				{
					...
					{
						CursorWidth = 0;
						for (i = pObj->CursorPos;
							 i < (int) (pObj->CursorPos + pObj->SelSize);
							 i++)
						{
					...
					{
						CursorOffset = GUI_UC__NumChars2NumBytes(pText,
																 pObj->CursorPos);
						Char = GUI_UC_GetCharCode(pText + CursorOffset);
						CursorWidth = GUI_GetCharDistX(Char);
					}
	...
	WM_ITERATE_START(NULL)
	{
		...
		if (pObj->Widget.State & WIDGET_STATE_FOCUS)
		{
			GUI_InvertRect(rInvert.x0,
						   rInvert.y0,
						   rInvert.x0 + CursorWidth - 1,
						   rInvert.y1);
		}
		...
		if (pObj->hpText)
		{
			...
			switch (pObj->Props.Align & GUI_TA_HORIZONTAL)
			{
				...
				case GUI_TA_RIGHT:
					xPos -= xSize -
							TextWidth -
							(pObj->Props.Border + EDIT_XOFF);
					break;
				default:
					xPos -= (pObj->Props.Border + EDIT_XOFF) +
							pObj->Widget.pEffect->EffectSize;
			}
			NumChars = GUI__GetNumChars(pText);
			if (xPos < 0)
			{
				...
				switch (Key)
				{
					case GUI_KEY_UP:
						if (pObj->hpText)
						{
							...
							U16 Char;
							pText = (char *) GUI_ALLOC_h2p(pObj->hpText);
							pText += GUI_UC__NumChars2NumBytes(pText,
															   pObj->CursorPos);
							Char = GUI_UC_GetCharCode(pText);
							if (Char < 0x7f)
							{
