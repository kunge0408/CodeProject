...
*/
static int _GetNumCharsInPrompt(const MULTIEDIT_OBJ *pObj,
								const char GUI_UNI_PTR *pText)
{
	char *pString, *pEndPrompt;
	int r = 0;
	pString = (char *) GUI_ALLOC_h2p(pObj->hText);
	pEndPrompt = pString + GUI_UC__NumChars2NumBytes(pString,
													 pObj->NumCharsPrompt);
	if (pText < pEndPrompt)
	{
...
*/
static int _WrapGetNumCharsDisp(MULTIEDIT_HANDLE hObj,
								const MULTIEDIT_OBJ *pObj,
								const char GUI_UNI_PTR *pText)
{
	int xSize, r;
...
*/
static int _WrapGetNumBytesToNextLine(MULTIEDIT_HANDLE hObj,
									  const MULTIEDIT_OBJ *pObj,
									  const char *pText)
{
	int xSize, r;
...
static int _GetCharDistX(const MULTIEDIT_OBJ *pObj, const char *pText)
{
	int r;
	if ((pObj->Flags & MULTIEDIT_SF_PASSWORD) &&
		(_GetNumCharsInPrompt(pObj, pText) == 0))
	{
...
*   	_DispString
*/
static void _DispString(MULTIEDIT_HANDLE hObj,
						const MULTIEDIT_OBJ *pObj,
						const char *pText,
						GUI_RECT *pRect)
{
...
* given line number.
*/
static char * _GetpLine(MULTIEDIT_HANDLE hObj,
						MULTIEDIT_OBJ *pObj,
						unsigned LineNumber)
{
	char * pText, *pLine;
...
*/
static int _GetCursorLine(MULTIEDIT_HANDLE hObj,
						  const MULTIEDIT_OBJ *pObj,
						  const char *pText,
						  int CursorPosChar)
{
...
*   	_GetCursorXY
*/
static void _GetCursorXY(MULTIEDIT_HANDLE hObj, /*const*/
						 MULTIEDIT_OBJ *pObj,
						 int *px,
						 int *py)
{
...
static void _ManageAutoScrollV(MULTIEDIT_HANDLE hObj, MULTIEDIT_OBJ *pObj)
{
	...
	{
		char IsRequired = _GetNumVisLines(hObj, pObj) < _GetNumLines(hObj,
																	 pObj);
...
*/
static int _CalcNextValidCursorPos(MULTIEDIT_HANDLE hObj,
								   MULTIEDIT_OBJ *pObj,
								   int CursorPosChar,
								   int *pCursorPosByte,
								   int *pCursorLine)
...
*/
static void _SetCursorPos(MULTIEDIT_HANDLE hObj,
						  MULTIEDIT_OBJ *pObj,
						  int CursorPosChar)
{
	int CursorPosByte, CursorLine;
	CursorPosChar = _CalcNextValidCursorPos(hObj,
											pObj,
											CursorPosChar,
											&CursorPosByte,
											&CursorLine);
...
*/
static void _SetCursorXY(MULTIEDIT_HANDLE hObj,
						 MULTIEDIT_OBJ *pObj,
						 int x,
						 int y)
{
	...
	if (pObj->hText)
	{
		...
		Char = GUI_UC__GetCharCode(pLine +
								   GUI_UC__NumChars2NumBytes(pLine, WrapChars));
		if (pObj->Flags & MULTIEDIT_SF_PASSWORD)
		{
			if (!Char)
			{
...
static int _IsOverwriteAtThisChar(MULTIEDIT_HANDLE hObj, MULTIEDIT_OBJ *pObj)
{
	...
	if (pObj->hText && !(pObj->Flags & MULTIEDIT_CF_INSERT))
	{
		...
		CurPos = _CalcNextValidCursorPos(hObj,
										 pObj,
										 pObj->CursorPosChar + 1,
										 0,
										 0);
...
static void _DeleteChar(MULTIEDIT_HANDLE hObj, MULTIEDIT_OBJ *pObj)
{
	if (pObj->hText)
	{
		...
		if (CursorOffset < strlen(s))
		{
			...
			_InvalidateCursorXY(pObj); /* Invalidate X/Y position */
			_ClearCache(pObj);
			pObj->CursorLine = _GetCursorLine(hObj,
											  pObj,
											  s,
											  pObj->CursorPosChar);
		}
	...
	if (pObj->hText)
	{
		...
		do
		{
			...
			if (pObj->CacheFirstVisibleLine != ScrollPosY)
			{
				if (Line == ScrollPosY)
				{
					pObj->CacheFirstVisibleByte = pText -
												  (const char *)
												  GUI_ALLOC_h2p(pObj->hText);
		...
		while (GUI_UC_GetCharCode(pText) &&
			   ((Line - ScrollPosY) <= NumVisLines));
	}
...
*   	_OnTouch
*/
static void _OnTouch(MULTIEDIT_HANDLE hObj,
					 MULTIEDIT_OBJ *pObj,
					 WM_MESSAGE *pMsg)
{
	int Notification;
	...
	if (pMsg->Data.p)
	{
		...
		if (pState->Pressed)
		{
			...
			yPos = pState->y +
				   pObj->ScrollStateV.v * GUI_GetYDistOfFont(pObj->pFont) -
				   Effect;
			_SetCursorXY(hObj, pObj, xPos, yPos);
			_Invalidate(hObj);
									...
									const char *pText)
{
	...
	hObj = WM_CreateWindowAsChild(x0,
								  y0,
								  xsize,
								  ysize,
								  hParent,
...
void MULTIEDIT_SetPrompt(MULTIEDIT_HANDLE hObj, const char *pPrompt)
{
	if (hObj)
	{
		...
		if (_IsCharsAvailable(pObj, NumCharsNew - NumCharsOld))
		{
			...
			{
				pText = (char *) GUI_ALLOC_h2p(pObj->hText);
				memmove(pText + NumBytesNew,
						pText + NumBytesOld,
						strlen(pText + NumBytesOld) + 1);
				if (pPrompt)
				{
