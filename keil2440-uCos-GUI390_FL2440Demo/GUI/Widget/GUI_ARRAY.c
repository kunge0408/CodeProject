...
int GUI_ARRAY_AddItem(GUI_ARRAY *pThis, const void *pNew, int Len)
{
	...
	if (r == 0)
	{
		...
		hNewBuffer = GUI_ALLOC_Realloc(pThis->haHandle,
									   (NumItems + 1) * sizeof(WM_HMEM));
		if (hNewBuffer == 0)
		{
			GUI_DEBUG_ERROROUT("GUI_ARRAY_AddItem failed to alloc buffer");
...
*   	an object, the destructor of the object is not called.
*/
WM_HMEM GUI_ARRAY_SetItem(GUI_ARRAY *pThis,
						  unsigned int Index,
						  const void *pData,
						  int Len)
{
...
{
	GUI_DEBUG_ERROROUT_IF(pThis->InitState != GUI_ARRAY_STATE_NOT_CREATED,
						  "GUI_ARRAY_Create: GUI_ARRAY not initialized to 0");
