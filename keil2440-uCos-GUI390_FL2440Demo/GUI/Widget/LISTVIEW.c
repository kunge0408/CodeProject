...
*/
static unsigned _GetNumVisibleRows(LISTVIEW_Handle hObj,
								   const LISTVIEW_Obj *pObj)
{
...
static void _Paint(LISTVIEW_Handle hObj, LISTVIEW_Obj *pObj, WM_MESSAGE *pMsg)
{
	...
	EndRow = pObj->ScrollStateV.v +
			 (((NumVisRows + 1) > NumRows) ? NumRows : NumVisRows + 1);
				...
				for (j = 0; j < NumColumns; j++)
				{
					...
					if (Rect.x1 >= ClipRect.x0)
					{
						...
						if (pItem->hItemInfo)
						{
							...
							pItemInfo = (LISTVIEW_ITEM_INFO *)
										GUI_ALLOC_h2p(pItem->hItemInfo);
						...
						Rect.x1 -= RBorder;
						Align = *((int *)
								  GUI_ARRAY_GetpItem(&pObj->AlignArray,
															 j));
...
*   	_SetSelFromPos
*/
static void _SetSelFromPos(LISTVIEW_Handle hObj,
						   LISTVIEW_Obj *pObj,
...
*   	_OnTouch
*/
static void _OnTouch(LISTVIEW_Handle hObj,
					 LISTVIEW_Obj *pObj,
					 WM_MESSAGE *pMsg)
{
	int Notification;
			...
			switch (pMsg->Data.v)
			{
				...
				case WM_NOTIFICATION_VALUE_CHANGED:
					...
					else if (pMsg->hWinSrc == WM_GetScrollbarH(hObj))
					{
						...
						LISTVIEW__UpdateScrollParas(hObj, pObj);
						HEADER_SetScrollPos(pObj->hHeader,
											pObj->ScrollStateH.v);
						_NotifyOwner(hObj, WM_NOTIFICATION_SCROLL_CHANGED);
					}
					break;
...
void LISTVIEW_AddRow(LISTVIEW_Handle hObj, const GUI_ConstString *ppText)
{
	if (hObj)
	{
		...
		if (GUI_ARRAY_AddItem(&pObj->RowArray, NULL, sizeof(GUI_ARRAY)) == 0)
		{
			...
			GUI_ARRAY* pRow;
			const char* s;
			GUI_ARRAY_CREATE((GUI_ARRAY *)
							 GUI_ARRAY_GetpItem(&pObj->RowArray, NumRows));  /* For higher debug levels only */
			/* Add columns for the new row */
			NumColumns = HEADER_GetNumItems(pObj->hHeader);
			for (i = 0; i < NumColumns; i++)
			{
				LISTVIEW_ITEM * pItem;
				pRow = (GUI_ARRAY *) GUI_ARRAY_GetpItem(&pObj->RowArray,
														NumRows);
				s = (ppText) ? *ppText++ : 0;
				if (s == 0)
				{
