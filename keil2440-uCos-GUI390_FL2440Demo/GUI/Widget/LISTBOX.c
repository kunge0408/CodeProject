...
*   	_CallOwnerDraw
*/
static int _CallOwnerDraw(LISTBOX_Handle hObj,
						  const LISTBOX_Obj *pObj,
						  int Cmd,
						  int ItemIndex)
{
...
{
	const char* s = NULL;
	LISTBOX_ITEM* pItem = (LISTBOX_ITEM *)
...
*   	_GetItemSizeX
*/
static int _GetItemSizeX(LISTBOX_Handle hObj,
						 const LISTBOX_Obj *pObj,
						 unsigned Index)
{
	LISTBOX_ITEM* pItem;
...
*   	_GetItemSizeY
*/
static int _GetItemSizeY(LISTBOX_Handle hObj,
						 const LISTBOX_Obj *pObj,
						 unsigned Index)
{
	LISTBOX_ITEM* pItem;
...
*   	_GetItemPosY
*/
static int _GetItemPosY(LISTBOX_Handle hObj,
						const LISTBOX_Obj *pObj,
						unsigned Index)
{
		...
		case WIDGET_ITEM_GET_YSIZE:
			{
				LISTBOX_Obj* pObj;
				pObj = LISTBOX_H2P(pDrawItemInfo->hWin);
				return GUI_GetYDistOfFont(pObj->Props.pFont) +
					   pObj->ItemSpacing;
			}
		case WIDGET_ITEM_DRAW:
			{
				...
				else
				{
					...
					else
					{
						...
						{
							ColorIndex = (pObj->Widget.State & WIDGET_STATE_FOCUS) ?
										 2 :
										 1;
						}
						else
					...
					(ItemIndex == pObj->Sel))
				{
					...
					LCD_SetColor(GUI_WHITE -
								 pObj->Props.aBackColor[ColorIndex]);
...
*/
void LISTBOX__InvalidateItem(LISTBOX_Handle hObj,
							 const LISTBOX_Obj *pObj,
							 int Sel)
{
	if (Sel >= 0)
...
*/
void LISTBOX__InvalidateItemAndBelow(LISTBOX_Handle hObj,
									 const LISTBOX_Obj *pObj,
									 int Sel)
{
	if (Sel >= 0)
...
static void _OnPaint(LISTBOX_Handle hObj, LISTBOX_Obj *pObj, WM_MESSAGE *pMsg)
{
	...
	GUI_MoveRect(&ClipRect,
				 -pObj->Widget.Win.Rect.x0,
				 -pObj->Widget.Win.Rect.y0);
...
*   	_GetItemFromPos
*/
static int _GetItemFromPos(LISTBOX_Handle hObj,
						   LISTBOX_Obj *pObj,
						   int x,
						   int y)
{
...
static int _OnMouseOver(LISTBOX_Handle hObj,
						LISTBOX_Obj *pObj,
						WM_MESSAGE *pMsg)
{
	const GUI_PID_STATE* pState = (const GUI_PID_STATE *) pMsg->Data.p;
	if (pObj->hOwner)
	{
		if (pState)
		{
			...
			{
				if (Sel <
					(int) (pObj->ScrollStateV.v + _GetNumVisItems(pObj, hObj)))
				{
...
static void _LISTBOX_Callback(WM_MESSAGE *pMsg)
{
	...
	switch (pMsg->MsgId)
	{
		...
		case WM_PID_STATE_CHANGED:
			{
				const WM_PID_STATE_CHANGED_INFO* pInfo = (const WM_PID_STATE_CHANGED_INFO *)
														 pMsg->Data.p;
				if (pInfo->State)
				{
					int Sel;
...
static int _AddKey(LISTBOX_Handle hObj, int Key)
{
	...
	switch (Key)
	{
		...
		case GUI_KEY_RIGHT:
			if (WM_SetScrollValue(&pObj->ScrollStateH,
								  pObj->ScrollStateH.v +
								  pObj->Props.ScrollStepH))
			{
				LISTBOX_UpdateScrollers(hObj);
				LISTBOX__InvalidateInsideArea(hObj);
			}
...
void LISTBOX_AddString(LISTBOX_Handle hObj, const char *s)
{
	if (hObj && s)
	{
		...
		WM_LOCK();
		pObj = LISTBOX_H2P(hObj);
		if (GUI_ARRAY_AddItem(&pObj->ItemArray,
							  &Item,
							  sizeof(LISTBOX_ITEM) + strlen(s)) == 0)
		{
