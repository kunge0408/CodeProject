...
print_mem_stats(j_common_ptr cinfo, int pool_id)
{
	...
	for (lhdr_ptr = mem->large_list[pool_id];
		 lhdr_ptr != NULL;
	...
	for (shdr_ptr = mem->small_list[pool_id];
		 shdr_ptr != NULL;
...
METHODDEF(void FAR *) alloc_large(j_common_ptr cinfo,
								  int pool_id,
								  size_t sizeofobject) /* Allocate a "large" object */
{
						  my_mem_ptr mem = (my_mem_ptr) cinfo->mem;
						  ...
						  hdr_ptr = (large_pool_ptr)
									jpeg_get_small/*RS: Changed from jpeg_get_large*/ (cinfo,
																					   sizeofobject +
																					   SIZEOF(large_pool_hdr));
						  if (hdr_ptr == NULL)
						  {
						  ...
						  mem->total_space_allocated += sizeofobject +
														SIZEOF(large_pool_hdr);
...
METHODDEF(void) realize_virt_arrays(j_common_ptr cinfo)
{
						 ...
						 sptr = sptr->next)
					{
						...
						{
							/* if not realized yet */
							space_per_minheight += (long)
												   sptr->maxaccess * (long)
												   sptr->samplesperrow * SIZEOF(JSAMPLE);
							maximum_space += (long)
											 sptr->rows_in_array * (long)
											 sptr->samplesperrow * SIZEOF(JSAMPLE);
						}
					}
					for (bptr = mem->virt_barray_list;
						 bptr != NULL;
						 bptr = bptr->next)
					{
						 ...
						 boolean writing)
{
					 ...
					 i += ptr->rowsperchunk)
				{
					...
					rows = MIN((long) ptr->rowsperchunk,
										 ...
										 boolean writable)
{
						  ...
						  if (ptr->first_undef_row < end_row)
						  {
							  ...
							  if (ptr->pre_zero)
							  {
								  size_t bytesperrow = (size_t)
													   ptr->samplesperrow *SIZEOF(JSAMPLE);
								  undef_row -= ptr->cur_start_row; /* make indexes relative to buffer */
								  end_row -= ptr->cur_start_row;
								  while (undef_row < end_row)
								  {
									  jzero_far((void FAR *)
												ptr->mem_buffer[undef_row],
												bytesperrow);
									  undef_row++;
								  }
							  }
										  ...
										  boolean writable)
{
						   ...
						   if (ptr->first_undef_row < end_row)
						   {
							   ...
							   {
								   size_t bytesperrow = (size_t)
														ptr->blocksperrow *SIZEOF(JBLOCK);
								   undef_row -= ptr->cur_start_row; /* make indexes relative to buffer */
								   end_row -= ptr->cur_start_row;
								   while (undef_row < end_row)
								   {
...
free_pool(j_common_ptr cinfo, int pool_id)
{
	...
	if (pool_id == JPOOL_IMAGE)
	{
		...
		for (sptr = mem->virt_sarray_list; sptr != NULL; sptr = sptr->next)
		{
			if (sptr->b_s_open)
			{
				...
				(*sptr->b_s_info.close_backing_store)
				(cinfo, & sptr->b_s_info);
			}
		}
		...
		for (bptr = mem->virt_barray_list; bptr != NULL; bptr = bptr->next)
		{
			if (bptr->b_s_open)
			{
				...
				(*bptr->b_s_info.close_backing_store)
				(cinfo, & bptr->b_s_info);
			}
		}
	...
	while (lhdr_ptr != NULL)
	{
		...
		space_freed = lhdr_ptr->hdr.bytes_used +
					  ...
					  SIZEOF(large_pool_hdr);
		jpeg_free_small/*RS: Changed from jpeg_free_large*/(cinfo,
															(void FAR *)
															lhdr_ptr,
															space_freed);
