...
/* Do computations that are needed before master selection phase */
{
	...
	{
		/* Provide 1/8 scaling */
		cinfo->output_width = (JDIMENSION) jdiv_round_up((long)
														 cinfo->image_width,
														 8L);
		cinfo->output_height = (JDIMENSION) jdiv_round_up((long)
														  cinfo->image_height,
														  8L);
...
prepare_for_output_pass(j_decompress_ptr cinfo)
{
	...
	if (cinfo->progress != NULL)
	{
		...
		{
			cinfo->progress->total_passes += (cinfo->enable_2pass_quant ?
											  2 :
											  1);
		}
	}
