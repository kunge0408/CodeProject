...
METHODDEF(void) process_data_context_main JPP((j_decompress_ptr cinfo,
											   JSAMPARRAY output_buf,
											   JDIMENSION *out_row_ctr,
											   JDIMENSION out_rows_avail));
#ifdef QUANT_2PASS_SUPPORTED
METHODDEF(void) process_data_crank_post JPP((j_decompress_ptr cinfo,
											 JSAMPARRAY output_buf,
											 JDIMENSION *out_row_ctr,
											 JDIMENSION out_rows_avail));
#endif
 ...
 */
{
		 ...
		 ci++, compptr++)
	{
		...
		rows_left = (int)
					(compptr->downsampled_height % (JDIMENSION) iMCUheight);
		if (rows_left == 0)
		{
		...
		if (ci == 0)
		{
			pMain->rowgroups_avail = (JDIMENSION)
						  ...
						  JDIMENSION out_rows_avail)
{
	...
	if (!pMain->buffer_full)
	{
		if (!(*cinfo->coef->decompress_data)
			(cinfo, pMain->xbuffer[pMain->whichptr]))
		{
			return;
		}			/* suspension forced, can do nothing more */
		pMain->buffer_full = TRUE;	/* OK, we have an iMCU row to work with */
