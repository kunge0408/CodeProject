...
*   	DrawNoTrans
*/
static void DrawNoTrans(int x0,
						int y0,
						int XSize,
						int YSize,
						int BytesPerLine,
					  ...
					  const U8 *pData)
{
	...
	for (y = 0; y < YSize; y++)
	{
		...
		for (x = 0; x < XSize - 1; x += 4)
		{
			...
			LCD_SetPixelAA(x + x0 + 1,
						   y0 + y,
						   aConvTable[((*pData0) >> 4) & 0x03]);
			LCD_SetPixelAA(x + x0 + 2,
						   y0 + y,
						   aConvTable[((*pData0) >> 2) & 0x03]);
			LCD_SetPixelAA(x + x0 + 3,
						   y0 + y,
						   aConvTable[((*pData0++)) & 0x03]);
		}
		if (Rem)
		{
			LCD_SetPixelAA(x + x0, y0 + y, aConvTable[((*pData0) >> 6)]);
...
*   	GUIPROP_FindChar
*/
static const GUI_FONT_PROP * GUIPROP_FindChar(const GUI_FONT_PROP *pProp,
											  U16P c)
{
	for (pProp = GUI_Context.pAFont->p.pProp;
		 pProp;
		 pProp = (const GUI_FONT_PROP*) pProp->pNext)
	{
...
void GUIPROP_AA2_DispChar(U16P c)
{
	...
	const GUI_FONT_PROP* pProp = GUIPROP_FindChar(GUI_Context.pAFont->p.pProp,
												  c);
	if (pProp)
	{
