...
static void _ShowHiResPixels(void) {
  ...
  GUI_DispStringHCenterAt("antialised", 160, 80);
  GUI_SetColor(GUI_BLUE);
  GUI_DispStringHCenterAt("antialised\nwith high\nresolution", 255, 80);
  hWindow = WM_CreateWindow(35, 120, 250, 60, WM_CF_SHOW | WM_CF_MEMDEV, &_cbWindow, 0);
  ...
  for(j=0;j<2;j++) 
  {
    for (i=0, angle=0; i<360; i++) {
      ...
      while (((GUI_GetTime()-tm) < 20) || (WM_Exec1() != 0));
    }
  }
...
void GUIDEMO_AA_Hirespixels(void) {
